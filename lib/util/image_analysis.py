...
def get_color_count(image, sort = False):
    ...
    colors = im.getcolors()
    if (colors is None): colors = im.getcolors(256*256)
    if (colors is None): raise Exception('can not get all colors of the image, raise maximum value or reduce image colors')
    if (sort == True): colors.sort(key=lambda tup:tup[0])
...
def get_color_classes(image, color_classes = [], threshold = 10):
    """ takes a list of color classes tuples (name, color) and assign the image colors with the given threshold to these classes """
    result = {}
